...
class Distance:
    ...
    def get_meters(self):
        if self._is_valid is True:
            return self.to_meters(float(self.dist_src), self.dist_uom)

    def check_distance(self):
        """ Distance validation. Uses float() function to check if distance value is a number """
        try:
            dist = float(self.dist_src)
            if self.allow_negative is False:
                if dist < 0:  # Check if distance is less than 0
                    self._is_valid = False
                    self._err_msg = 'Distance error.\n'
                else:
                    self._is_valid = True
            else:
                self._is_valid = True
...
""" Bearing, magnetic variation section """
